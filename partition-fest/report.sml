...
= struct
  ...
  fun make (T.Leaf sIds) _ = map (fn sId => (sId, [])) sIds
                      ...
                      let val g = Map.lookup (sid, grades)
                          fun gradeEvidence h =
                              ...
                              in  if similar = total
                                  ...
                                  else real similar / real (similar - total)
                              end
                      in  fobs gradeEvidence obs observationMap
                      end
                  fun biggerObs (obs, NONE) = SOME obs
                    | biggerObs (obs0, SOME obs1) = if weight obs0 > weight obs1
                                                    then SOME obs0
                                                    else SOME obs1
                  val (tmark, _) = valOf (enumerate db biggerObs NONE)
                  val report = (tmark, ReportUtil.describe db (sid, tmark))
              in  SolutionMap.insert (m, sid, [report])
              end
